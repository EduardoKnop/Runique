...
class RoomAnalyticsRepository(
    ...
    override suspend fun getAnalyticsValues(): AnalyticsValues {
        return withContext(Dispatchers.IO) {
            ...
            val avgDistancePerRunOverTime = async {
                val list = mutableListOf<Pair<LocalDate, Double>>()
                analyticsDao.getAvgDistancePerRunOverTime().forEach {
                    list += Pair(LocalDate.parse(it.date), it.distanceMeters)
                }
            ...
            AnalyticsValues(
                ...
                avgDistancePerRunOverTime = avgDistancePerRunOverTime.await()
            )
        }
    }
}